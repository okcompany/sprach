...
const AIMatchingExerciseSchema = z.object({
  ...
  pairs: z.array(AIMatchingPairSchema).describe("Пары слов."), 
...
const AIAudioQuizExerciseSchema = z.object({
  ...
  items: z.array(AIAudioQuizItemSchema).describe("Аудио-задания."), 
});

// --- Zod Schemas for Grammar Exercises ---
const FillInTheBlanksQuestionSchema = z.object({
  promptText: z.string().describe("Предложение на немецком с пропусками для заполнения. Необязательные подсказки для пропусков (на немецком) могут быть в скобках ()."),
  correctAnswers: z.array(z.string()).min(1).describe("Правильный ответ(ы) на немецком."),
...
const FillInTheBlanksExerciseSchema = z.object({
  ...
  questions: z.array(FillInTheBlanksQuestionSchema).describe("Вопросы на заполнение пропусков."), 
});

const MultipleChoiceQuestionSchema = z.object({
  questionText: z.string().describe("Вопрос или предложение на немецком с пропуском."),
  options: z.array(z.string()).min(2).describe("2-4 варианта ответа на немецком."), 
...
const MultipleChoiceExerciseSchema = z.object({
  ...
  questions: z.array(MultipleChoiceQuestionSchema).describe("Вопросы с множественным выбором."), 
});

const SentenceConstructionTaskSchema = z.object({
  words: z.array(z.string()).min(3).describe("Слова на немецком для составления предложения."),
  possibleCorrectSentences: z.array(z.string()).min(1).describe("Правильное(ые) предложение(я) на немецком."),
...
const SentenceConstructionExerciseSchema = z.object({
  ...
  tasks: z.array(SentenceConstructionTaskSchema).describe("Задания на составление предложений."), 
});
...
const ListeningExerciseSchema = z.object({
  ...
  questions: z.array(z.string()).describe("Открытые вопросы на понимание на русском или немецком (в зависимости от того, на каком языке ожидается ответ)."), 
});
...
const AIComprehensionMultipleChoiceQuestionSchema = z.object({
  ...
  options: z.array(z.string()).min(2).describe("2-4 варианта ответа."), 
  correctAnswer: z.string().describe("Правильный вариант ответа."),
  explanation: z.string().optional().describe("Необязательное объяснение на русском."),
});
...
const AIComprehensionMultipleChoiceExerciseSchema = z.object({
  ...
  questions: z.array(AIComprehensionMultipleChoiceQuestionSchema).describe("Вопросы."), 
});
...
const AITrueFalseExerciseSchema = z.object({
  ...
  statements: z.array(AITrueFalseStatementSchema).describe("Утверждения."), 
});
...
const AISequencingExerciseSchema = z.object({
  ...
  shuffledItems: z.array(z.string()).describe("Перемешанные элементы для упорядочивания (на немецком)."), 
...
const lessonPrompt = ai.definePrompt({
  ...
  2. OPTIONAL Interactive Vocabulary Exercises: Provide AT MOST ONE of the following:
     - "interactiveMatchingExercise": Provide "instructions" (in Russian), "pairs" (array of {german, russian}), "germanDistractors" (optional, 1-2 strings), "russianDistractors" (optional, 1-2 strings).
  ...
  3. OPTIONAL Interactive Listening Exercises: Provide AT MOST ONE of the following. This exercise should be based on the main "listeningExercise.script". Instructions in Russian.
  ...
  4. OPTIONAL Interactive Reading Exercises: Provide AT MOST ONE of the following. This exercise should be based on the main "readingPassage". Instructions in Russian.
