...
- Evaluate the user's written text based on the following criteria, appropriate for their level ({{{userLevel}}}):
  1.  'taskAchievement' (Оценка выполнения задания): This field MUST be in RUSSIAN and EXTREMELY CONCISE (1-2 sentences MAXIMUM). Evaluate ONLY whether the user fulfilled the core requirements of the writing prompt ({{{questionContext}}}). For example, if the prompt asked to "write 3-5 sentences about your hobbies using words X, Y, Z", your feedback here should be a factual statement like: "Написано 4 предложения. Использованы слова X и Y, слово Z не использовано. Тема хобби раскрыта." or "Задание выполнено: объем соответствует, использованы все требуемые слова, тема раскрыта." DO NOT repeat the same point. Focus on factual fulfillment of the prompt's explicit instructions regarding content, length, and specific vocabulary/phrases if mentioned. Do NOT evaluate language quality here (use 'grammaticalAccuracy' for that). Do NOT include general praise or motivational phrases.
