...
const WritingEvaluationDetailsSchema = z.object({
  taskAchievement: z.string().optional().describe("Feedback on how well the user addressed the writing prompt. (Оценка выполнения задания. ДОЛЖНО БЫТЬ НА РУССКОМ, ОЧЕНЬ КРАТКО, СТРУКТУРИРОВАНО ПО ПУНКТАМ С НОВОЙ СТРОКИ)"),
...
- Evaluate the user's written text based on the following criteria, appropriate for their level ({{{userLevel}}}):
  1.  'taskAchievement' (Оценка выполнения задания): This field MUST be in RUSSIAN.
      **CRITICAL INSTRUCTIONS FOR 'taskAchievement':**
      a.  Structure your response for 'taskAchievement' using the following labels, EACH ON A NEW LINE (use '\\n' for newlines). After each label, provide a VERY BRIEF (1 short sentence) factual assessment for the user's level.
          - Полнота ответа: [Assess if all parts of the prompt in {{{questionContext}}} were addressed. E.g., "Задание выполнено полностью, все аспекты {{{questionContext}}} учтены." or "Не все пункты задания {{{questionContext}}} были освещены."]
          - Логическая связность: [Briefly comment on the logical flow. E.g., "Мысли изложены логично." or "Логика повествования немного нарушена."] (This is a general, brief assessment for this field; detailed feedback is in 'coherenceAndCohesion' field if needed)
          - Связность текста: [Briefly comment on text cohesion. E.g., "Текст в целом связан." or "Связь между некоторыми предложениями слабая."] (This is a general, brief assessment for this field; detailed feedback is in 'coherenceAndCohesion' field if needed)
          - Структурированность: [Briefly comment on overall structure. E.g., "Текст хорошо структурирован." or "Структура текста могла бы быть четче."] (This is a general, brief assessment for this field; detailed feedback is in 'coherenceAndCohesion' field if needed)
          - Грамматическая корректность (обзорно): [Provide an EXTREMELY brief, high-level comment on overall grammatical correctness FOR THE USER'S LEVEL. E.g., "Грамматика в целом соответствует уровню." or "Некоторые грамматические моменты требуют внимания (см. детали в разделе 'Оценка грамматической правильности')." DO NOT list specific errors here.]
          - Следование инструкциям (общий вывод): [Provide a VERY brief overall statement about instruction adherence. E.g., "Основные инструкции задания выполнены." or "Не все инструкции учтены (см. 'Полнота ответа')." This should be a summary.]
      b.  For "Полнота ответа", focus EXCLUSIVELY on whether the user *did what was explicitly asked* in {{{questionContext}}} (e.g., length, specific words if requested, topic, format).
      c.  **DO NOT re-list or rephrase the requirements** from the original writing prompt ({{{questionContext}}}) under any label.
      d.  DO NOT evaluate language quality (grammar, style) in detail here for "Логическая связность", "Связность текста", "Структурированность", "Грамматическая корректность (обзорно)", "Следование инструкциям (общий вывод)". Use other fields like 'grammaticalAccuracy', 'lexicalResource', or 'coherenceAndCohesion' for detailed analysis of those aspects. The comments here should be high-level and very brief for each point.
      e.  DO NOT add praise, general comments, or motivational phrases. Be factual and concise for each point.
      f.  Ensure the entire 'taskAchievement' string is still reasonably short, even with the newlines and labels. The goal is clear, structured, brief feedback for this specific field. Each of the 6 points above should be one very short sentence.
      g.  **Example of a GOOD 'taskAchievement' output string:**
          "Полнота ответа: Задание выполнено полностью, все вопросы из {{{questionContext}}} затронуты.\\nЛогическая связность: Мысли изложены в целом логично.\\nСвязность текста: Предложения связаны между собой.\\nСтруктурированность: Текст имеет вступление и заключение.\\nГрамматическая корректность (обзорно): Соответствует уровню A1.\\nСледование инструкциям (общий вывод): Инструкции выполнены."
      h.  **Example of a BAD 'taskAchievement' output string (DO NOT DO THIS):**
          "Полнота ответа: Пользователь должен был написать 3-5 предложений. Пользователь должен был использовать слова X, Y, Z. Пользователь написал о выходных. Это очень важно, так как развивает навыки...\nЛогическая связность: Текст логичен, потому что пользователь следовал теме, и это хорошо.\nСвязность текста: Текст связан, так как предложения следуют друг за другом, что замечательно.\nСтруктурированность: Текст имеет структуру, молодец!\nГрамматическая корректность (обзорно): Есть несколько ошибок, например, в падежах и артиклях, но в целом неплохо для этого уровня.\nСледование инструкциям (общий вывод): Пользователь пытался следовать инструкциям, но не все получилось, нужно быть внимательнее." (This is too verbose, re-lists requirements, gives detailed grammar feedback here, and doesn't follow the conciseness rule for each point).
  2.  'coherenceAndCohesion' (Оценка связности и логичности текста): This field MUST be in RUSSIAN. Provide a more DETAILED evaluation of the text's logical structure, flow, and use of linking words IF there are specific issues or good examples to highlight. Otherwise, a brief comment is fine. No praise or generic comments. Keep this section concise (1-2 sentences if no major issues, more if specific feedback is needed).
...
Output your response in the following JSON format (ensure grammarErrorTags is an array of strings, or omit if not applicable; ensure writingDetails and errorExplanationDetails are objects or omit if not applicable):
{
  ...
  "grammarErrorTags": [],
  "writingDetails": {
    "taskAchievement": "Полнота ответа: Задание выполнено полностью, все аспекты задания учтены.\\nЛогическая связность: Мысли изложены логично.\\nСвязность текста: Текст связан.\\nСтруктурированность: Текст хорошо структурирован.\\nГрамматическая корректность (обзорно): Соответствует уровню.\\nСледование инструкциям (общий вывод): Инструкции выполнены.",
